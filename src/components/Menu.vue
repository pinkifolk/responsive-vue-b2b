<template>
  <div>
    <v-app-bar app color="white">
      ...
      <v-toolbar-title @click="inicio" style="cursor: pointer"
        >B2B Provaltec</v-toolbar-title
      >
    ...
    >
      <div v-for="data in getUserData" :key="data.index">
        ...
        <v-layout align-center>
          <v-flex>
            <p class="ml-5 mb-0 white--text caption">
              Ejecutivo: {{ data.vendedor }}
            </p>
            <p class="ml-5 mb-2 white--text caption">
      ...
      <v-list>
        <!-- menu primary-->
        <v-list-group v-for="item in menu" :key="item.id">
          ...
          >
            ...
            <v-list-item v-for="sub in child.menu" :key="sub.title">
              <v-list-item-content>
                <router-link
                  :to="{ name: 'productos', params: { id: sub.id } }"
        ...
        <!-- Secondary menu -->
        <v-list-group v-for="item in menuSecondary" :key="item.title" no-action>
          <template v-slot:activator>
            <v-list-item-content>
              <v-list-item-title v-text="item.title"></v-list-item-title>
            </v-list-item-content>
          ...
          <v-list-item v-for="child in item.menu" :key="child.title">
            <v-list-item-content>
              <router-link
                :to="{ name: 'productos', params: { id: child.id } }"
                style="text-decoration: none"
              >
                <v-list-item-title
                  v-text="child.title"
...
export default {
  data: () => ({
    drawer: false, // switch show/hide menu
    menu: [
      {
        id: 1,
        ...
        menu: [
          {
            title: "Valvula Aguja",
            menu: [{ id: 3, title: "Aguja Hilo NPT" }],
          },
          {
          ...
          {
            title: "Valvula Pinch",
            menu: [{ id: 139, title: "Valvula Pinch ANSI" }],
          },
          {
          ...
          },
          {
            ...
            menu: [{ id: 101, title: "Solenoide Hilo NPT" }],
          },
          {
    ...
    ], // hard industrial valves menu (pending for improvements)
    menuSecondary: [
      {
        id: 45,
    ...
    ], // secondary hard menu, this contains bronze, filter, fitting and flange, fire network, accessories and water filtration (pending for improvements)
    getUserData: [], // array showing user info in side menu
  }),
  mounted() {
  ...
  methods: {
    /*
          ----------------------------------------------------------------	
              Function: salir
              Description: This function logs off the application,
              deletes the data stored in the localstorage and sends us
              at the beginning.
          ----------------------------------------------------------------
        */
    salir() {
    ...
    /*
          ----------------------------------------------------------------	
              Function: inicio
              Description: This function is a shortcut to reach the
              dashboard that is the start after entering the system
          ----------------------------------------------------------------
        */
    inicio() {
    ...
    /*
          ----------------------------------------------------------------	
              ...
              Description: This function activates the side menu when the
    ...
    /*
          ----------------------------------------------------------------	
              ...
              the user's data, obtains the name + last name,
              the assigned executive and the discount he has.
          ----------------------------------------------------------------
        */
    async getUser() {
      const id = localStorage.getItem("user");
      await axios
        .get("/api/user/sales/" + id)
        .then((response) => {
